---
title: "Add PayNow Recipient | DBS Singapore"
source_url: "https://www.dbs.com.sg/personal/support/bank-local-funds-transfer-add-paynow-recipient.html"
scraped_date: "2026-04-06 00:05:33"
depth: 2
---

## Add PayNow Recipient

Add a PayNow recipient instantly using DBS digibank.

*Part of: Guides \>* [Your Guide to digibank](guide-ibanking.html), [Your Guide to PayNow](guide-paynow.html)

### Important information

- The Payee/Recipient must have a **PayNow Profile** to be added to your list of payees.
- You may add up to a maximum of **50 Individuals** in your list of payees.

### How to add PayNow recipient

There are various channels which you may add PayNow recipient with us. The most convenient method would be via digibank mobile.

- Log in to digibank mobile with your **Touch / Face ID** or **digibank User ID \& PIN**.
- Tap **Pay \& Transfer** and select **PayNow**.
- Select **Recipient** and tap **Add PayNow Recipient**.
- Select the **PayNow ID type ( Mobile Number, NRIC/FIN, UEN or VPA)** from the dropdown list.
- Enter the recipient's relevant details, verify the **Recipients's name** and tap **Next**.
- Verify the details, tap **Add now** and complete the **2\-Factor Authentication** to add your PayNow recipient.

  

**Don't have digibank?**  

Get the latest digibank mobile app now!

##### Already a digibank user?

[Login now](https://internet-banking.dbs.com.sg/digi-web/ngib/)
Forgot [User ID](https://internet-banking.dbs.com.sg/digi-web/ngib/forgot-userid) or [PIN](https://internet-banking.dbs.com.sg/digi-web/ngib/forgot-pin)

##### Not yet a digibank user?

[Get Started](https://www.dbs.com.sg/personal/hoap/registration?brand=dbs&entry=IBNextgen)
*For customers with ATM/Debit/Credit Card*

**Was this information useful?**
Yes
No

## We welcome your feedback


 Information is easy to understand and find
 

Yes

No


 Information is useful to answer your inquiries completely
 

Yes

No

### Thanks for your feedback!

#### Weâll continue to work towards serving you better.

#### Related articles

- [Transfer Funds using PayNow](bank-local-funds-transfer-paynow-transfer.html)
- [Remove PayNow Recipient](bank-local-funds-transfer-remove-paynow-recipient.html)

#### Related articles

- [Transfer Funds using PayNow](bank-local-funds-transfer-paynow-transfer.html)
- [Remove PayNow Recipient](bank-local-funds-transfer-remove-paynow-recipient.html)